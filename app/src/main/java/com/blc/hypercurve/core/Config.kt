...
/**
 * 模块 UI 与 hook 侧共用的配置定义。
 *
 * 传输通道：libxposed remote preferences（框架侧只读，UI 侧通过 XposedService 写入）。
 */
object Config {
     ...
     *
     * 与 [KEY_CAP] 的区别：cap 夹住请求（限高），本项抬高系统天花板（解锁）。
     */
    const val DEFAULT_MANUAL_MAX = 0

    /** 系统默认手动上限参考：普通 0.3165（≈5185）、阳光模式 0.411977（≈6750）。 */
    ...
    /**
     * 默认 nit <-> backlight 样条（来自 dumpsys display 的 mNits / mBacklight），
     * 供界面「换算预览」把码值换算成 nit。
     */
